...
market_data = pd.DataFrame({
	'DD': np.log(dd['Close']) / np.log(dd['Volume']),
	'DJIA': np.log(djia['Close']) / np.log(djia['Volume']),
	'DJTA': np.log(djta['Close']) / np.log(djta['Volume']),
	'SPX': np.log(spx['Close']) / np.log(spx['Volume']),
	# 'W5000': wilshire['Close'] / wilshire['Volume'],
	# 'VIX': vix['Close'],
});
...
X = market_data[['DJIA_delta', 'DJTA_delta', 'SPX_delta']];
y = market_data['DD_delta'];


k_fold = StratifiedKFold(n_splits=25, shuffle=True, random_state=14);

svm_rbf = SVC(kernel='rbf', probability=True);
# svm_rbf = SVC(kernel=rbf_linear_kernel, probability=True);
rbf_scores = cross_val_score(svm_rbf, X, y, cv=k_fold, scoring='roc_auc');


svm_rbf.fit(X,y);
y_predict = svm_rbf.predict_proba(X)[:,1];
false_positive_rate, true_positive_rate, cutoffs = roc_curve(y, y_predict);
